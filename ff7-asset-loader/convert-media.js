...
const createCombinedMoviesList = async () => {
  const allMovies = await fs.readJson('../metadata/movie-list/movie-list-combined.json')
  console.log('allMovies', allMovies)
  let nonFieldVideos = (await fs.readdir(config.outputMoviesDirectory)).filter(f => f.includes('.mp4')).map(f => f.replace('.mp4', '')).filter(f => !allMovies.includes(f))
...
const createCombinedMusicList = async () => {
  ...
  await fs.writeJson('../metadata/music-list/music-list-combined.json', musicList, { spaces: '\t' })
  console.log(musicList)
}
const extractMedias = async () => {
  await extractSounds()
  await createCombinedMusicList()
  await extractMusic()
  await extractMovies()
  await extractMoviecamData()
  await createCombinedMoviesList()
}
const init = async () => {
  extractMedias()
}
