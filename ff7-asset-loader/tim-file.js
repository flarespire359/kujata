...
class TimFile {
  // https://github.com/myst6re/makoureactor/blob/4645b4b0595626b04163d3d0fa2d4b7569d6b440/core/TimFile.cpp
  ...
  loadTimFileFromBuffer (buffer) {
    ...
    if (this.tim.header.isCLUT) {
      ...
      this.tim.clut.noOfPalettes =
        (this.tim.clut.length - 12) / (this.tim.clut.paletteSize * 2)
      ...
      for (let i = 0; i < this.tim.clut.noOfPalettes; i++) {
        ...
        for (let j = 0; j < this.tim.clut.paletteSize; j++) {
          ...
          const color = getColorForPalette(colorBytes)
    ...
    if (this.tim.header.bpp === '4bit') {
      this.tim.imageData.width = this.tim.imageData.width * 4
    }
    if (this.tim.header.bpp === '8bit') {
      this.tim.imageData.width = this.tim.imageData.width * 2
    }
    ...
    switch (this.tim.header.bpp) {
      ...
      case '16bit':
        // console.log('16bit')
        while (r.length > r.offset) {
          const color = getColorForPalette(r.readUShort())
      ...
      case '24bit': // Should validate this
        // console.log('24bit')
        totalImageData =
          this.tim.imageData.width * this.tim.imageData.height * 2 // Not sure about 2
  ...
  async saveAsPngWithPalette (outputPath, paletteId) {
    const channels = 4
    const n = this.tim.imageData.width * this.tim.imageData.height * channels // Seems to require 2x width also
    // console.log('-----')
    ...
    switch (this.tim.header.bpp) {
      case '4bit':
        for (let i = 0; i < this.tim.imageData.data.length; i++) {
          ...
          data[i * 8 + 3] =
            palette[pixelIdA].r === 0 &&
            palette[pixelIdA].g === 0 &&
            palette[pixelIdA].b === 0
              ? 0x00
              : palette[pixelIdA].a
          ...
          data[i * 8 + 7] =
            palette[pixelIdB].r === 0 &&
            palette[pixelIdB].g === 0 &&
            palette[pixelIdB].b === 0
              ? 0x00
              : palette[pixelIdB].a
        }
        break
    ...
    await sharp(Buffer.from(data.buffer), {
      raw: {
        width: this.tim.imageData.width,
        height: this.tim.imageData.height,
        channels: 4
      }
    }).toFile(outputPath)
  }
