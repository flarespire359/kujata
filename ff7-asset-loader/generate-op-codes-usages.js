...
const generateOpCodeUsages = (config) => {
  ...
  const fieldFiles = fs.readdirSync(config.outputFieldFLevelDirectory).filter(f => f.endsWith('.json'))
  //   console.log('fieldFiles', fieldFiles)
  const opCodeUsages = {}
  for (const fieldIndex in fieldFiles) {
    const fieldFile = fieldFiles[fieldIndex]
    const field = fs.readJSONSync(path.join(config.outputFieldFLevelDirectory, fieldFile))
  ...
  for (const opHex of Object.keys(opCodeUsages)) {
    ...
    const filename = config.metadataDirectory + '/op-code-usages/' + opHex + '.json'
